...
btn1 = ft.ElevatedButton(style = button_style, content = ft.Text(data.answer, size = 25), on_click = click)
btn2 = ft.ElevatedButton(style = button_style, content = ft.Text(data.wrong_answer, size = 25), on_click = click)
...
progressBar = ft.ProgressBar(height = 5, width = 500, color = "#70b658", bgcolor = "#eb4540")
number_of_question = ft.Text(f"{data.index + 1} / {len(data.file)}", size=18)

UI = (banner, question_text, row, counter, progressBar, number_of_question)
